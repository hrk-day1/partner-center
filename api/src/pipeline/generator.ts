...
function partitionSpecLinesForSteps(
  description: string,
  maxStepLines: number,
): {
  stepLines: string[];
...
function buildExpectedFromSpec(intent: string, description: string, maxSpecLines: number): string {
  ...
  return merged.length <= EXPECTED_RESULT_MAX_LEN ? merged : `${intent} (스펙 설명의 주요 항목이 반영된다)`;
}
...
function determinePriority(domain: string, type: TcType, rules: ResolvedPriorityRule[]): Priority {
...
function determineSeverity(domain: string, type: TcType, rules: ResolvedSeverityRule[]): Severity {
...
export function generateTestCases(
  ...
  const domainCounts = Object.fromEntries(resolved.domainOrder.map((d) => [d, emptyDomainTypeCounts()])) as Record<
    string,
    Record<TcType, number>
  >;

  for (const item of checklist) {
    const templates = resolved.templates[item.domain] ?? [];
    const applicableTemplates = config.maxTcPerRequirement ? templates.slice(0, config.maxTcPerRequirement) : templates;
...
function baseActionLabelsForPointType(tp: TestPoint): string[] {
  switch (tp.pointType) {
    case '정상조회':
      return ['조회 실행', '목록/상세 데이터 확인'];
    case '빈결과':
      return ['데이터 없는 조건 설정', '조회 실행', '빈 상태 메시지 확인'];
    case '필터정렬':
      return ['필터/정렬 조건 설정', '적용', '결과 정합성 확인'];
    case '페이지네이션':
      return ['첫 페이지 확인', '다음 페이지 이동', '마지막 페이지 확인'];
    case '정상등록':
      return ['유효 데이터 입력', '저장', '결과 확인'];
    case '필수값누락':
      return ['필수 필드 비움', '저장 시도', '오류 메시지 확인'];
    case '중복등록':
      return ['동일 데이터로 등록 시도', '중복 처리 결과 확인'];
    case '입력경계값':
      return ['최소 길이 입력', '최대 길이 입력', '경계 초과 입력', '결과 확인'];
    case '정상수정':
      return ['데이터 변경', '저장', '변경 반영 확인'];
    case '필수값검증':
      return ['필수 필드 비움', '저장 시도', '오류 확인'];
    case '취소재진입':
      return ['수정 중 취소', '재진입', '원래 값 유지 확인'];
    case '정상삭제':
      return ['대상 선택', '삭제 실행', '목록에서 제거 확인'];
    case '삭제확인':
      return ['삭제 버튼 클릭', '확인 다이얼로그 표시 확인', '확인/취소 동작 검증'];
    case '참조무결성':
      return ['참조 중인 데이터 삭제 시도', '차단 또는 경고 확인'];
    case '유효전이':
      return ['현재 상태 확인', '허용된 상태로 전이 실행', '전이 결과 확인'];
    case '비허용전이':
      return ['현재 상태 확인', '비허용 상태로 전이 시도', '차단 확인'];
    case '전이후반영':
      return ['상태 전이 실행', 'UI 반영 확인', '데이터 반영 확인'];
    case '승인처리':
      return ['승인 대상 선택', '승인 실행', '상태 변경 및 후속 처리 확인'];
    case '반려처리':
      return ['반려 대상 선택', '반려 사유 입력', '반려 실행', '상태 변경 확인'];
    case '권한없는승인':
      return ['권한 없는 계정으로 로그인', '승인 시도', '차단 확인'];
    case '허용역할':
      return ['허용 역할로 로그인', '대상 기능 접근', '정상 동작 확인'];
    case '비허용역할':
      return ['비허용 역할로 로그인', '대상 기능 접근 시도', '차단 메시지 확인'];
    case '숨김차단':
      return ['권한 없는 계정으로 로그인', '메뉴/버튼 숨김 또는 비활성 확인'];
    case '정상업로드':
      return ['허용 형식 파일 선택', '업로드 실행', '성공 확인'];
    case '비허용파일':
      return ['비허용 형식 또는 초과 크기 파일 선택', '업로드 시도', '거부 메시지 확인'];
    case '다운로드검증':
      return ['업로드된 파일 다운로드', '원본과 동일 여부 확인'];
    case '정상결제':
      return ['유효 결제수단 선택', '결제 실행', '승인 결과 확인'];
    case '결제실패':
      return ['실패 조건 설정 (잔액 부족 등)', '결제 시도', '오류 메시지 확인'];
    case '환불처리':
      return ['환불 요청', '환불 처리 확인', '금액 반환 확인'];
    case '금액경계값':
      return ['최소 금액 결제', '최대 금액 결제', '경계 초과 결제 시도'];
    case '중복결제방지':
      return ['동일 요청으로 결제 2회 시도', '중복 차단 확인'];
    case '정상실행':
      return ['배치 예약 설정', '예약 시간 도래', '실행 결과 확인'];
    case '실패재시도':
      return ['배치 실행 실패 유도', '재시도 또는 알림 확인'];
    case '중복실행방지':
      return ['동일 배치 중복 실행 시도', '방지 동작 확인'];
    case '정상연동':
      return ['외부 시스템 호출 실행', '응답 및 데이터 반영 확인'];
    case '연동실패':
      return ['외부 시스템 장애 상황 설정', '호출 실행', '오류 처리 확인'];
    case '데이터정합성':
      return ['연동 실행', '내부-외부 데이터 비교'];
    default:
      return [tp.intent];
  }
}
