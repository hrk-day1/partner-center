...
const WRITE_FEATURE_TYPES: ReadonlySet<FeatureType> = new Set(['등록', '수정', '삭제', '파일처리']);
...
export function buildKeywordPatterns(resolved: ResolvedSkill): Map<string, RegExp> {
...
export function tryInferDomain(text: string, patterns: Map<string, RegExp>, resolved: ResolvedSkill): string | null {
  for (const domain of resolved.domainOrder) {
...
function inferDomain(text: string, patterns: Map<string, RegExp>, resolved: ResolvedSkill): string {
...
export type TcSourceFieldRow = Record<'대분류' | '중분류' | '소분류' | '기능명' | '기능설명', string>;
...
function buildFeatureName(
  ...
  const seq = [pick(indices.cat1), pick(indices.cat2), pick(indices.cat3), pick(indices.feature)];
  const parts: string[] = [];
  for (const s of seq) {
    if (!s) continue;
...
function buildCategoryPath(row: string[], indices: { cat1: number; cat2: number; cat3: number }): string {
  const parts: string[] = [];
...
export function buildChecklist(
  ...
  if (col.feature < 0) {
    console.warn("[plan] 헤더에 '기능명'에 해당하는 열이 없습니다. 행 스킵이 늘 수 있습니다.");
  }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const featureDescription = featureDescIdx >= 0 ? filledRow[featureDescIdx]?.trim() || '' : '';
    ...
    const reqId = col.reqId >= 0 ? filledRow[col.reqId]?.trim() || `AUTO-${rowNum}` : `AUTO-${rowNum}`;

    const precondition = col.precondition >= 0 ? filledRow[col.precondition]?.trim() || '' : '';
