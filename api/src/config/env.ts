...
export const env = {
  ...
  llmPlanConcurrency: Math.min(8, Math.max(1, Number(process.env.LLM_PLAN_CONCURRENCY) || 3)),
  ...
  highRiskMaxTcPerRequirementDefault: Math.max(2, Number(process.env.PIPELINE_HIGH_RISK_MAX_TC_PER_REQUIREMENT) || 6),
} as const;
