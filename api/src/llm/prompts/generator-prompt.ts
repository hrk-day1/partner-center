...
function buildOmittableCompactKeysSection(config: { environmentDefault: string; ownerDefault: string }): string {
  return `
...
function formatTestPointsForPrompt(entries: ChecklistWithTestPoints[]): string {
  return entries
    .map((e) => {
      const risk =
        e.item.specRiskTier === 'high' ? '높음 (금전·거래·규제 근거 스펙 — 단계·데이터·Expected 상세)' : '일반';
...
function formatPolicyHints(hints: PolicyHint[], domain: string): string {
  ...
  const lines = relevant.map((h) => `  - [${h.riskLevel ?? 'medium'}] ${h.hint}`).join('\n');
