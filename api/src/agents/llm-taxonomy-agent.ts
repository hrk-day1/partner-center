...
import {
  buildTaxonomyDomainDetailPrompt,
  buildKeywordRefillPrompt,
} from '../llm/prompts/taxonomy-domain-detail-prompt.js';
import {
...
async function runHybridLlmPhase(
  ...
  const prompt = buildHybridTaxonomyPrompt(presetResolved, unclassifiedRows, input.headers, input.sourceSheetName);
  ...
  const validReclassified = hybridResult.reclassified.filter((r) => presetResolved.domainOrder.includes(r.domain));

  const existingIds = new Set(presetResolved.domainOrder);
  const validNewDomains = hybridResult.newDomains.filter((nd) => !existingIds.has(nd.id) && domainIdRegex.test(nd.id));
  ...
  if (validNewDomains.length > 0) {
    ...
    const newDomainOrder = [...presetResolved.domainOrder, ...validNewDomains.map((d) => d.id)] as readonly string[];
...
/** @deprecated 폴백 경로; 현재 호출부 없음. 필요 시 orchestrator에서 연결. */
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- 상기 폴백 보존
async function runFullTaxonomyPhase(
  ...
  const results = await Promise.allSettled(domainOrder.map((id) => fetchDomain(id)));
  ...
  const { domains: dedupedDomainsInit, removedCount } = dedupeKeywordsByDomainOrder(mergedDomains);
  let dedupedDomains = dedupedDomainsInit;
  if (removedCount > 0) {
  ...
  if (deficitDomains.length > 0) {
    const allUsedKeys = new Set(dedupedDomains.flatMap((d) => d.keywords.map((k) => k.toLowerCase().trim())));
...
export async function runTaxonomyPhase(
  ...
  if (unclassifiedRows.length === 0) {
    ...
    console.log(
      `[taxonomy] ${pipelineId} ${unclassifiedRows.length}/${totalRows} rows unclassified, running hybrid LLM`,
    );

    const hybridResult = await runHybridLlmPhase(input, presetResolved, unclassifiedRows, bus, pipelineId, agentId);
  ...
  console.log(
    `[taxonomy] ${pipelineId} domains=${resolved.domainOrder.join(',')} ${Date.now() - start}ms tokens=${totalTokens}`,
  );
