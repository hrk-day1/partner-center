...
export class DeterministicTaxonomyEvaluatorAgent implements Agent<TaxonomyEvaluatorInput, TaxonomyEvaluationResult> {
  ...
  async run(
    ...
    try {
      ...
      bus.emit(config.pipelineId, {
        ...
        status: 'completed',
        progress: 100,
        message: result.passed ? 'Taxonomy 검증 통과' : `Taxonomy 검증 이슈 ${result.issues.length}건`,
        timestamp: new Date().toISOString(),
