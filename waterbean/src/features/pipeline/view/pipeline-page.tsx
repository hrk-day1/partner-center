        ...
        <div className="space-y-1 text-sm text-zinc-600">
          ...
          {selectedSkill && (
            <p className="text-xs text-zinc-500">
              {t('pipeline.info.selectedSkill')} <span className="font-medium text-zinc-700">{selectedSkill.name}</span>{' '}
              &mdash; {selectedSkill.description}
            </p>
          )}
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="mt-1 flex min-h-[1.25rem] items-center gap-2 text-xs text-zinc-500">
              ...
              {!sheetMetaLoading && sheetMetaError && <span className="text-danger">{sheetMetaError}</span>}
              {!sheetMetaLoading && !sheetMetaError && sheetName && <span>{t('pipeline.sheetMeta.hint')}</span>}
            </div>
            {submitSheetError && <p className="text-danger mt-1 text-xs">{t('pipeline.sheetName.required')}</p>}
          </div>
