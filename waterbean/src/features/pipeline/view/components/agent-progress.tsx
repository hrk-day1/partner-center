        ...
        {agents.map((agent) => {
          ...
          return (
            <div key={agent.agentId} className="flex items-center gap-3">
              <Icon className={`h-4 w-4 shrink-0 ${color} ${agent.status === 'running' ? 'animate-spin' : ''}`} />
              <div className="min-w-0 flex-1">
                ...
                <p
                  className={`mt-0.5 text-xs text-zinc-600 ${isOrchestrator ? 'break-words whitespace-pre-wrap' : 'truncate'}`}
                >
                  {agent.message}
                </p>
                {detail && (
