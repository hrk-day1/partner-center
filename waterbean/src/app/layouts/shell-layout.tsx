...
export function ShellLayout() {
  ...
  const navClass = ({ isActive }: { isActive: boolean }) =>
    cn(
      ...
      isActive ? 'bg-accent/15 text-accent' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900',
    );
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-zinc-50/50">
      ...
      {!asideOpen && (
        <button
          type="button"
          onClick={openAside}
          className="border-border fixed top-3 left-3 z-30 flex h-9 w-9 items-center justify-center rounded-lg border bg-white text-zinc-700 shadow-sm transition-colors hover:bg-zinc-50"
          aria-expanded={false}
      ...
      <div className="flex min-h-0 flex-1 basis-0">
        <aside
          id="app-aside-nav"
          className={cn(
            'border-border flex flex-col border-r bg-white duration-200 ease-out',
            isMdUp
              ? cn(
              ...
              : cn(
                  ...
                  asideOpen ? 'translate-x-0' : 'pointer-events-none -translate-x-full',
                ),
          )}
        ...
        >
          <div className="flex h-full min-h-0 min-w-56 flex-col">
            ...
            <nav className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto p-3" aria-label={t('layout.navLabel')}>
